...
const TeamCard = (props) => {
  ...
  for (let i=props.list.length; i<2; ++i){
    userSkeleton.push(
      ...
      <ListItem className={classes.listItem}>
          ...
          <ListItemText
            primary={<Skeleton width="100%"><Typography>.</Typography></Skeleton>} 
  ...
  return(
    <Badge badgeContent={props.ready ? <CheckCircleIcon /> : ''} className={classes.readyBadge}>
    <Card className={classes.card} color="white" raised variant={props.team !== TEAMS.NEUTRAL ? "outlined" : "elevation"} style={{"borderColor":color}} >
      <CardContent>
        <LinearProgress variant="determinate" value={props.percentOfMembers} color={themeColor}/>
        <List>
          {props.spymaster !== '' ?
            <>
              <ListItem className={classes.listItem}>
                <ListItemAvatar>
                  <Avatar alt={props.spymaster.user} src={`https://api.adorable.io/avatars/50/${props.spymaster.id}.png`} />
                </ListItemAvatar>
                <ListItemText 
            ...
            :
          <>
            <Box display="flex" alignItems="center" justifyItems="center" justifyContent="center" width="100%">
              <Button
                variant="text"
                color={themeColor}
                size="large"
          ...
          {props.list.map((user, i) => {
            return (
              props.spymaster?.id !== user.id ?
                <ListItem key={i} className={classes.listItem}>
                  <ListItemAvatar key={i}>
                      <Avatar alt={user.user} src={`https://api.adorable.io/avatars/50/${user.id}.png`} />
                  </ListItemAvatar>
                  <ListItemText primary={user.user} className={classes.username}  primaryTypographyProps={{'noWrap':true}}/>
                </ListItem>
              : ''
            )
          })}
      ...
      <CardActions>
        ...
        {props.spymaster?.id === props.userId && props.allowReady ?
